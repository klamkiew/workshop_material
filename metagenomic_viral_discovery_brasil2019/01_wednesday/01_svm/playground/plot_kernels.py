...
with open('virus.csv', 'r') as inputStream: 
    ...
    for idx, row in enumerate(reader): 
        if idx == 0: 
            continue 
        # we only take the first two features, due to visualisation
        # further, we need to convert the strings into floats
...
for i, clf in enumerate((svc, lin_svc, rbf_svc, poly_svc)):
    ...
    plt.scatter(X[:, 0], X[:, 1], c=y, cmap=plt.cm.coolwarm, s=10, edgecolor='k', lw=1)
